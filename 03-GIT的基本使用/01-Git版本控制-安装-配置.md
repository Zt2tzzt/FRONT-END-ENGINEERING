...
在 [Git 的官网](https://git-scm.com/downloads)，根据自己的操作系统，下载 Git 即可；

在 window 操作系统按照默认配置全局安装即可；

git 安装的 Bash；CMD；GUI 分别是什么？
...
- 每台计算机上，只需要配置一次，程序升级时会保留配置信息；
- 可以在任何时候，再次通过运行命令，来修改它们；

### 2.配置 Git 外观和行为的变量

Git 自带一个 `git config` 工具，来帮助配置 Git 外观和行为的变量：
...
每一个 Git 提交，都会使用这些信息，它们会写入到你的每一次提交中，不可更改；
...
git config --global --unset core.autocrlf # 消除 core.autocrlf 设置
```
...
方式一：初始化一个 Git 仓库，并且可以将当前项目的文件，都添加到 Git 仓库中（目前很多的脚手架在创建项目时，都会默认创建一个 Git 仓库）；
...
- 但是，在这个时候，我们仅仅是做了一个初始化的操作，你的项目里的文件，还没有被跟踪；

方式二：从其它服务器克隆（clone）一个已存在的 Git 仓库（第一天到公司通常我们需要做这个操作）；
