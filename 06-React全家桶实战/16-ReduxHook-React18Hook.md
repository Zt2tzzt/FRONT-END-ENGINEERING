...
在之前的 redux 开发中，为了让 react 和 redux 结合起来，我们使用了 _react-redux_ 中的 `connect`：
...
09-learn-reacthooks\src\13-redux 中的 hooks\store\features\counter.js
...
09-learn-reacthooks\src\13-redux 中的 hooks\store\index.js
...
09-learn-reacthooks\src\13-redux 中的 hooks\App.jsx
...
其中包括了 `useSelector`、`useDispatch` 和 `useStore` 3 个 Hook。
...
09-learn-reacthooks\src\13-redux 中的 hooks\App-hooks.jsx
...
const App = memo(props => {
  ...
  function changeNumberHandle(num, isAdd = true) {
    if (isAdd) dispatch(addNumberAction(num))
    else dispatch(subNumberAction(num))
  }
...
单页面富应用（SPA）存在的 2 个缺陷（反之是 SSR 的 2 个优势）：

因为浏览器渲染页面的流程是：域名 -> dns 服务器 -> ip 地址 -> 访问服务器获取静态资源 `index.html`；

然而，SPA 项目中的 `index.html`，通常没有实质的内容。这样造成了 2 个缺陷：
...
09-learn-reacthooks\src\14-useId 的使用\App.jsx
...
09-learn-reacthooks\src\15-useTransition 和 useDeferredValue\namesArray.js
...
09-learn-reacthooks\src\15-useTransition 和 useDeferredValue\01-useTransition 的使用.jsx
...
09-learn-reacthooks\src\15-useTransition 和 useDeferredValue\02-useDeferredValue 的使用.jsx
