...
*Redux Toolkit* 库可解决以上问题，它是官方推荐的编写 Redux 逻辑的库，又称为“RTK”
...
- `configureStore`：包装 `createStore` 以提供简化的配置选项和良好的默认值。
  ...
  - 提供的任何 Redux 中间件，默认包含了 *redux-thunk* 中间件，
...
- `name`：用户标记 slice 的名词。
	- 这样浏览器插件 *redux-devtool* 中会显示对应的名词；
- `initialState`：初始状态值；
...
在之前的开发中，我们通过 *redux-thunk* 中间件让派发的 action 中可以进行异步操作。
...
为了节约内存和性能消耗，其中实现了一个算法：*Persistent Data Structure*（持久化数据结构或一致性数据结构）； 
- 用一种数据结构来保存数据； 
