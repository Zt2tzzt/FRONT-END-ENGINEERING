...
在执行事件函数时，有可能我们需要获取一些参数信息：比如 event 对象、以及其他参数：
...
React 官方的说法：Virtual DOM 是一种编程理念，虚拟 DOM 帮助我们从命令式编程转到了声明式编程的模式。
...
- 不同的项目，在这个模板的基础之上进行开发或者进行一些配置的简单修改即可；
...
React 脚手架默认是基于 Webpack 来开发的；但是，很奇怪：我们并没有在目录结构中看到任何 webpack 相关的内容？
