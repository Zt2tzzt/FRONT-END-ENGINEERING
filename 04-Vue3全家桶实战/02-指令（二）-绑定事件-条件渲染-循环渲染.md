...
- 修饰符：
  ...
  ```html
  <button @click.stop="btnClick">按钮</button> <input type="text" @keyup.enter="enterKeyup" />
  ```
...
- 方法不写 `()`，默认传递 $event 参数。
- 如果既要用 `$event` 也要传入参数，方法应要写成 `foo($event, arg)`；$event 实参位置不受限，也可写成 `foo(arg, $event)`。
...
<body>
  <div id="app">
    ...
    <input type="text" @keyup.enter="enterKeyup" /><!-- 输入时 enter 键抬起触发事件 -->
  </div>
  <script src="https://unpkg.com/vue@next"></script>
  <script>
    const app = {
...
- 因为 v-if 是一个指令，所以必须将其添加到一个元素上：
  - 但是如果我们希望切换的是多个元素呢？并且不希望包裹它们的元素被渲染出来。
...
- 使用 v-show 指令的元素会被渲染到 DOM，并使用 CSS 的 display 属性进行切换。
