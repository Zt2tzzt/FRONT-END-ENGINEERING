...
- 理解3种在前端控制权限的方法，
	...
	- 方法三：在前端获取用户权限能，如访问的菜单，根据用户权限**动态添加路由**。，这么做要求后端要返回需要动态添加的路由对应的组件，有2种方式：
...
6. 调用全局的 `beforeResolve` 守卫（**全局守卫**）(2.5+)。
   ...
   - 名字取得不好，类似与 beforeEach，但时机是在异步组件解析之后，跳转之前。能确保获取到自定义 `meta` 属性。
...
app.use(store) // app.use(store)时，加入了全局属性$store
app.mount('#app')
```
...
> Vuex的5大核心，state，getters，mutations，actions，modules。
...
template 中取 state，VOA 使用 `mapState` 辅助函数进行映射。2种写法。分别的使用场景。
...
template中取 getters，VOA 使用 `mapGetters` 辅助函数进行映射。2种写法。分别的使用场景。
...
将 mutations 中标识符值抽成常量，结合ES6对象增强语法计算属性名使用。这是一种设计规范，官方文档推荐这么做。
...
template 中取 mutations，VOA 使用 `mapMutations` 辅助函数进行映射。2种写法。分别的使用场景。
