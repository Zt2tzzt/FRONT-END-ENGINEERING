...
- 小程序中的 Mustache 只能写标识符和表达式，不能写函数。
...
<!-- 遍历字符串 -->
<view class="str">
  <view wx:for="coderzzt" wx:key="*this">
    {{ item }}
  </view>
</view>
```
...
- 属性以 `bind` 或 `catch` 开头, 从1.5.0版本开始, 可以在 `bind` 和 `catch` 后加上一个冒号； 
...
当视图层发生事件时，某些情况需要事件携带一些参数到执行的函数中，在小程序中，目前有2种方式，实现事件传递。
...
- 课题一：组件内的样式对外部样式的影响 
	...
	- 结论二：组件内使用 id 选择器、属性选择器、标签选择器会对外界产生影响）。
...
<view class="section">
  ...
  <view class="content info">{{ content }}</view> <!-- 使用接收的样式名 info -->
</view>
```
